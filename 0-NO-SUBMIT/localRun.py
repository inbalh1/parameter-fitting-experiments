...
def sample_and_measure(mode="all"):
    ...
    model = 'erdos-renyi'
    input_files = [os.path.splitext(os.path.basename(f))[0] for f in glob.glob(f"../output_data/fitted_params/{model}/*.csv")]
    if mode == "compact":
        input_files = input_files[:1]
    base_input = '../output_data/fitted_params/erdos-renyi'
    base_output = f"../output_data/fitted_features/{model}"
...
def fit_parameters(mode):
    ...
    model = 'erdos-renyi'
    base_input = f'../output_data/fitted_params/{model}'
    base_output = f"../output_data/fitted_features/{model}"
    input_files = [os.path.splitext(os.path.basename(f))[0] for f in glob.glob(f"{base_input}/*")]
    if mode == 'compact':
        input_files = input_files[:1]
